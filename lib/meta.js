...
(function () {
  ...
  window.RoverkMeta = {
    ...
    // «Ring meg opp»: brukeren la igjen navn og nummer. Samme hendelse som et klikk
    // på telefonnummeret (initContactTracking under), og bevisst uten verdi — et
    // nummer er ikke et solgt skur, og skal ikke telles som ett.
    contact: function (o) {
      o = o || {};
      if (!ready()) return Promise.resolve();
      loadPixel();
      return applyMatching(o.user).then(function () {
        track('Contact', {
          content_ids: [productId()],
          content_name: o.content_name || 'ring meg opp',
          currency: 'NOK'
        });
      });
    },
